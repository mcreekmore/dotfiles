...
$BREW_PATH/chezmoi init --apply https://github.com/mcreekmore/dotfiles.git

if [ "$STATUS" = "unauthenticated" ]; then
    echo "Bitwarden Master Password:"
    BW_SESSION=$($BREW_PATH/bw login --raw </dev/tty)
    export BW_SESSION="$BW_SESSION"
    echo "Successfully logged in"
elif [ "$STATUS" = "locked" ]; then
    echo "Bitwarden vault is locked. Please enter your master password to unlock:"
    BW_SESSION=$($BREW_PATH/bw unlock --raw </dev/tty)
    export BW_SESSION="$BW_SESSION"
